...
if (analogRead(L)>400 && analogRead(M)>400 && analogRead(R)>400)
{
  VAL=0;
}
else
{
  ...
  if (analogRead(M)>400)
  {
    servo_direction.write(servo_initial_position=74);
  }
  if (analogRead(L)>400)
  {
    servo_direction.write(servo_initial_position=54);
  }
  if (analogRead(R)>400)
  {
    servo_direction.write(servo_initial_position=94);
  }
}
